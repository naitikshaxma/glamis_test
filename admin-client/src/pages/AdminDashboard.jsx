...
const QuickActionCard = ({ title, description, href, emoji }) => (
  <Link to={href}>
    <Card className="p-5 rounded-xl shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 cursor-pointer border border-gray-100 h-full min-h-[120px] flex flex-col justify-center">
              ...
              return (
                <div key={item.label}>
                  <div className="flex justify-between items-center mb-1.5">
                    <Typography variant="small" className="font-medium text-gray-600">{item.label}</Typography>
          ...
          {stats?.upcomingInterviews?.length > 0 ? (
            <div className="space-y-3">
              {stats.upcomingInterviews.map((interview, index) => (
                <div key={interview._id || index} className="flex items-center justify-between p-3.5 bg-green-50 rounded-lg border border-green-100">
                  <div>
                    <Typography variant="small" className="font-bold text-green-900">{interview.name}</Typography>
